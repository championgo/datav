...
async download() {
    ...
    let headers = [
      [
        { t: 'ID', f: 'id', m1: 'A1', m2: 'A1' },
        { t: '文件名', f: 'file_name', m1: 'B1', m2: 'B1' },
        { t: '字段名', f: 'original_field', m1: 'C1', m2: 'C1' },
        { t: '英文', f: 'en', m1: 'C1', m2: 'C1' },
      ],
    ];
    let data = await this.ctx.service.order.download();
    await ctx.helper.exportExcel(data, headers, '多语言');
  }
  ...
  async uploadExcel() {
    ...
    try {
      ...
      const target = path.join(dirname, 'test.xlsx');
      if (!fs.existsSync(dirname)) fs.mkdirSync(dirname);
      const data = await ctx.helper.handleExcel(target);
      result = await ctx.service.order.uploadExcel(data);
 ...
 async updateOrder() {
    const ctx = this.ctx;
    const result = await ctx.service.order.updateOrder();
    ctx.status = 200;
    ctx.body = result;
  }
     ...
     async updateCount() {
    const ctx = this.ctx;
    const result = await ctx.service.order.updateCount();
    ctx.status = 200;
    ctx.body = result;
  }
...
async getLan() {
      const ctx = this.ctx;
    const result = await ctx.curl('http://agent1.ai.esehoo.cn/test.php');
    ctx.status = result.status;
    ctx.set(result.headers);
    const res = await ctx.service.order.getLan(JSON.parse(result.data));
    ctx.body = res;
  }
}
